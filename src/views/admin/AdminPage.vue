...
import { useGoodsStore, type Goods, GOODS_BG_PRESETS } from '@/store/goods'
        ...
        <div v-else class="panel">
          ...
          <div class="table-wrap">
            <base-table :columns="[
              ...
              { key: 'name', label: '商品名称' ,showTooltip: true},
              { key: 'emoji', label: '图片' ,showTooltip: true},
              { key: 'price', label: '现价',showTooltip: true },
              { key: 'originPrice', label: '原价',showTooltip: true },
              { key: 'flashPrice', label: '秒杀价',showTooltip: true },
              { key: 'sales', label: '销量',showTooltip: true },
              { key: 'bg', label: '背景色',showTooltip: true },
              { key: 'operate', label: '操作', width: 160 }
            ]" :table-data="goodsStore.goodsList.map((g) => ({
                ...g,
                actions: g
              }))" row-key="id">
              <template #operate>
